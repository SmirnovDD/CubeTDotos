...
//
//     public void Execute(Entity entity, ref CharacterControllerComponentData characterControllerComponentData,
...
//
//         {
//             CheckForObstacles(entity, ref characterControllerComponentData, collider, position, rotation,
//                 aiMovementData);
//
//             if (_obstacleIsInTheWay == true)
//             {
//                 TargetPos = _obstacleCollisionPoint +
//                             _obstacleCollisionNormal * aiMovementData.ObstacleAvoidanceDistance;
...
//     }
//
//     private void MoveToTarget(ref CharacterControllerComponentData characterControllerComponentData,
//         in Translation position, in AIMovementData aiMovementData)
//     {
...
//         characterControllerComponentData.CurrentDirection = math.lerp(characterControllerComponentData.CurrentDirection,
//             normalizedVectorToTarget, DeltaTime * aiMovementData.RotationSpeed);
//         characterControllerComponentData.CurrentMagnitude =
//             1; //squaredDistanceToTarget <= aiMovementData.SquaredStoppingDistance ? 0 : 1f;
//         //Debug.DrawLine(position.Value, TargetPos, Color.cyan);
//     }
//
//     private void CheckForObstacles(Entity entity, ref CharacterControllerComponentData controller,
//         in PhysicsCollider collider, in Translation position, in Rotation rotation, in AIMovementData aiMovementData)
//     {
...
//     }
//
//     private void CheckDifferentDirections(ref Entity entity, in PhysicsCollider collider, in Translation position,
//         in Rotation rotation, in AIMovementData aiMovementData)
//     {
//         _closestDistance = math.INFINITY;
//         for (int i = 0; i < ColliderCastDirections.Length; i++)
//         {
//             _obstacleCheckColliderCastPosition =
//                 position.Value + ColliderCastDirections[i] * aiMovementData.ObstacleAvoidanceDistance;
//             CastCollidersAtDirections(ref entity, collider, position, rotation, aiMovementData);
//         }
//     }
//
//     private void CastCollidersAtDirections(ref Entity entity, in PhysicsCollider collider, in Translation position,
//         in Rotation rotation, in AIMovementData aiMovementData)
//     {
//         quaternion currRot = rotation.Value;
//         float3 targetPos = _obstacleCheckColliderCastPosition;
//         NativeList<ColliderCastHit> horizontalCollisions = PhysicsUtilities.ColliderCastAll(collider, position.Value,
//             targetPos, CollisionWorld, entity, CollisionFilters.ObstacleAvoidanceCollider, Allocator.Temp);
//
//         if (horizontalCollisions.Length > 0)
//         {
//             NativeList<DistanceHit> horizontalDistances = PhysicsUtilities.ColliderDistanceAll(collider,
//                 aiMovementData.ObstacleAvoidanceDistance, new RigidTransform {pos = targetPos, rot = currRot},
//                 CollisionWorld, entity, CollisionFilters.ObstacleAvoidanceCollider, Allocator.Temp);
//
